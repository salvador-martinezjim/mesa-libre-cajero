...
const CURRENT_USER = {
  ...
  email: 'salva.martinez1210@gmail.com'
};
...
const INITIAL_TABLES: Table[] = [
  ...
  { id: 4, name: 'Mesa 4', location: 'Terraza Suprem', status: 'available', peopleCount: 0 }, 
  { id: 5, name: 'Mesa 5', location: 'Patio', status: 'available', peopleCount: 0 },
  { id: 16, name: 'Mesa 16', location: 'Terraza Suprem', status: 'available', peopleCount: 0 },
  { id: 18, name: 'Mesa 18', location: 'Patio', status: 'available', peopleCount: 0 },
  { id: 20, name: 'Mesa 20', location: 'Sin zona', status: 'available', peopleCount: 0 },
];
...
export const TablesPage: React.FC = () => {
  ...
  const getStatusStyles = (status: TableStatus) => {
    switch (status) {
      case 'occupied':
        return { bg: '#FFEBEB', text: '#FF4C4C', label: 'OCUPADA', border: '2px solid #FFE5E5' }; 
      case 'available':
        return { bg: '#E6F4EA', text: '#1E8E3E', label: 'DISPONIBLE', border: '2px solid #D1E7DD' }; 
      default:
          ...
          <div style={styles.modalOverlay}>
              <div style={styles.logoutModalContent}>
                  <div style={styles.logoutModalHeader}>
                      ...
                      <button style={styles.closeModalButton} onClick={() => setIsLogoutModalOpen(false)}>
                          <CloseIcon />
                      </button>
                  </div>
      ...
      <header style={styles.header}>
        <div style={{...styles.userInfo, cursor: 'pointer'}} onClick={() => setIsProfileOpen(true)}>
          ...
          <div>
            ...
            <p style={styles.userRole}>{CURRENT_USER.role}</p>
          </div>
        </div>
        ...
        <>
            <div style={styles.controlsContainer}>
                ...
                {FILTER_CATEGORIES.map(category => (
                    ...
                    style={{
                        ...styles.categoryPill,
                        backgroundColor: activeCategory === category ? '#FF9F43' : '#FFFFFF',
            ...
            <div style={styles.tablesGrid}>
                ...
                >
                    ...
                    <span style={styles.peopleCountText}>
                        {isAvailable ? '0 personas' : `${table.peopleCount} personas`}
                    </span>
                    </div>

                    <div style={styles.cardFooter}>
                        {isAvailable ? (
                            <span style={styles.waiterLabelEmpty}>Esperando asignación...</span>
                        ) : (
                            <>
                                <span style={styles.waiterLabel}>Atendido por:</span>
                                <div style={styles.waiterInfo}>
                                    <span style={styles.waiterNameText}>{table.waiterName}</span>
                                    {table.waiterAvatarUrl && <img src={table.waiterAvatarUrl} alt="Waiter" style={styles.smallAvatar} />}
                                </div>
                            </>
                        )}
                    </div>
                ...
                {filteredTables.length === 0 && (
                    <div style={styles.noResults}>
                        <p>No se encontraron mesas con estos filtros.</p>
                    </div>
                )}
            </div>
        </>
